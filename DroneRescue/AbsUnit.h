...
public:
	...
	AbsUnit(std::string name, std::string fp); // Ajout d'une interface pour definir explicitement le chemin
	...
	std::map<std::string, std::string> getProperties() const { return m_properties; }

	virtual void accept(class AbsUnitVisitor& v) = 0;
	virtual void accept(class AbsUnitVisitor& v) const = 0;
